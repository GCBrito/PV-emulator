...
ns      = 132;       % number of series cells
np      = 1;        % number of parallel branches

Vmp_mod_ref  = 40.3;     % voltage at maximum power point (V)
Imp_mod_ref  = 17.25;     % current at maximum power point (A)
Voc_mod_ref  = 48.3;     % open-circuit voltage (V)
Isc_mod_ref  = 18.28;     % short-circuit current (A)

Tref   = 25 + 273.15;       % reference temperature (°C)
Sref    = 1000;     % reference irradiance (W/m²)

alpha   = 0.0004;  % temperature coefficient of Isc (A/K)

%% Operating Conditions

T = 25 + 273.15; % Current temperature (K)
S = 200; % Current irradiance (W/m²)
